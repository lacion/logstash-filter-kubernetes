...
describe LogStash::Filters::KubernetesMetadata do
  ...
  describe "Get pod metadata with single log-format" do
    ...
    sample("path" => "/var/log/containers/kube-testwithsinglelogformat-abcde_default_myappname-47d3a3bfb112dbd2fd6e255e1e3d9eb91a10b62342e620e4917e2f5e24398507.log") do
      expect(subject['kubernetes']['pod']).to eq('kube-testwithsinglelogformat-abcde')
      expect(subject['kubernetes']['namespace']).to eq('default')
      expect(subject['kubernetes']['container_name']).to eq('myappname')
      expect(subject['kubernetes']['replication_controller']).to eq('kube-testwithsinglelogformat')
      expect(subject['kubernetes']['annotations']['log-format']).to be_kind_of(String)
      expect(subject['kubernetes']['log_format_stdout']).to eq('single_format')
      expect(subject['kubernetes']['log_format_stderr']).to eq('single_format')
      expect(subject['kubernetes']['labels']['app']).to eq('myappname')
    end
  ...
  describe "Get pod metadata with container name via log-format-stdout-container and log-format-stderr-container" do
    ...
    sample("path" => "/var/log/containers/kube-testwithcontainername-abcde_default_myappname-47d3a3bfb112dbd2fd6e255e1e3d9eb91a10b62342e620e4917e2f5e24398507.log") do
      expect(subject['kubernetes']['pod']).to eq('kube-testwithcontainername-abcde')
      expect(subject['kubernetes']['namespace']).to eq('default')
      expect(subject['kubernetes']['container_name']).to eq('myappname')
      expect(subject['kubernetes']['replication_controller']).to eq('kube-testwithcontainername')
      expect(subject['kubernetes']['annotations']['log-format-stdout-myappname']).to be_kind_of(String)
      expect(subject['kubernetes']['annotations']['log-format-stderr-myappname']).to be_kind_of(String)
      expect(subject['kubernetes']['log_format_stdout']).to eq('format_stdout')
      expect(subject['kubernetes']['log_format_stderr']).to eq('format_stderr')
      expect(subject['kubernetes']['labels']['app']).to eq('myappname')
    end
  ...
  describe "Get pod metadata with both log-format-stderr and log-format-stdout" do
    ...
    sample("path" => "/var/log/containers/kube-testwithlogformat-abcde_default_myappname-47d3a3bfb112dbd2fd6e255e1e3d9eb91a10b62342e620e4917e2f5e24398507.log") do
      expect(subject['kubernetes']['pod']).to eq('kube-testwithlogformat-abcde')
      expect(subject['kubernetes']['namespace']).to eq('default')
      expect(subject['kubernetes']['container_name']).to eq('myappname')
      expect(subject['kubernetes']['replication_controller']).to eq('kube-testwithlogformat')
      expect(subject['kubernetes']['annotations']['log-format-stdout']).to be_kind_of(String)
      expect(subject['kubernetes']['annotations']['log-format-stderr']).to be_kind_of(String)
      expect(subject['kubernetes']['log_format_stdout']).to eq('stdout_format')
      expect(subject['kubernetes']['log_format_stderr']).to eq('stderr_format')
      expect(subject['kubernetes']['labels']['app']).to eq('myappname')
    end
  ...
  describe "Get pod metadata with partial default" do
    ...
    (0..2).each do |i|
      sample("path" => "/var/log/containers/kube-testwithpartialdefault-abcde_default_myappname-47d3a3bfb112dbd2fd6e255e1e3d9eb91a10b62342e620e4917e2f5e2439850#{i}.log") do
        expect(subject['kubernetes']).to be_a(Hash)
        expect(subject['kubernetes']['pod']).to eq('kube-testwithpartialdefault-abcde')
        expect(subject['kubernetes']['namespace']).to eq('default')
        expect(subject['kubernetes']['container_name']).to eq('myappname')
        expect(subject['kubernetes']['replication_controller']).to eq('kube-testwithpartialdefault')
        expect(subject['kubernetes']['labels']['app']).to eq('myappname')
        expect(subject['kubernetes']['log_format_stderr']).to eq('default')
        expect(subject['kubernetes']['log_format_stdout']).to eq('stdout_format')
      end
    end
  end
  ...
  describe "Get pod metadata with no log-format" do
    ...
    sample("path" => "/var/log/containers/kube-testnologformat-abcde_default_myappname-47d3a3bfb112dbd2fd6e255e1e3d9eb91a10b62342e620e4917e2f5e24398507.log") do
      expect(subject['kubernetes']['pod']).to eq('kube-testnologformat-abcde')
      expect(subject['kubernetes']['namespace']).to eq('default')
      expect(subject['kubernetes']['container_name']).to eq('myappname')
      expect(subject['kubernetes']['replication_controller']).to eq('kube-testnologformat')
      expect(subject['kubernetes']['log_format_stdout']).to eq('default')
      expect(subject['kubernetes']['log_format_stderr']).to eq('default')
      expect(subject['kubernetes']['labels']['app']).to eq('myappname')
    end
  end
  ...
  describe "Get pod metadata with no bad character in annotations key name" do
    ...
    sample("path" => "/var/log/containers/kube-testbadcharinannotations-abcde_default_myappname-47d3a3bfb112dbd2fd6e255e1e3d9eb91a10b62342e620e4917e2f5e24398507.log") do
      expect(subject['kubernetes']['pod']).to eq('kube-testbadcharinannotations-abcde')
      expect(subject['kubernetes']['namespace']).to eq('default')
      expect(subject['kubernetes']['container_name']).to eq('myappname')
      expect(subject['kubernetes']['replication_controller']).to eq('kube-testbadcharinannotations')
      expect(subject['kubernetes']['annotations']['kubernetes.io/created-by']).to be_falsey
      expect(subject['kubernetes']['annotations']['kubernetes_io-created-by_']).to be_kind_of(String)
      expect(subject['kubernetes']['log_format_stderr']).to eq('default')
      expect(subject['kubernetes']['labels']['app']).to eq('myappname')
    end
  end
  ...
  describe "Get pod metadata with no bad character in label key name" do
    ...
    (0..2).each do |i|
      sample("path" => "/var/log/containers/kube-testbadcharinlabels-abcde_default_myappname-47d3a3bfb112dbd2fd6e255e1e3d9eb91a10b62342e620e4917e2f5e24398507.log") do
        expect(subject['kubernetes']['pod']).to eq('kube-testbadcharinlabels-abcde')
        expect(subject['kubernetes']['namespace']).to eq('default')
        expect(subject['kubernetes']['container_name']).to eq('myappname')
        expect(subject['kubernetes']['replication_controller']).to eq('kube-testbadcharinlabels')
        expect(subject['kubernetes']['labels']['test.label']).to be_falsey
        expect(subject['kubernetes']['labels']['test_label']).to be_kind_of(String)
        expect(subject['kubernetes']['log_format_stderr']).to eq('default')
        expect(subject['kubernetes']['labels']['app']).to eq('myappname')
      end
    end
  ...
  describe "Get pod metadata with no metadata" do
    ...
    (0..2).each do |i|
      sample("path" => "/var/log/containers/kube-testnometadata-abcde_default_myappname-47d3a3bfb112dbd2fd6e255e1e3d9eb91a10b62342e620e4917e2f5e24398507.log") do
        expect(subject['kubernetes']['pod']).to eq('kube-testnometadata-abcde')
        expect(subject['kubernetes']['namespace']).to eq('default')
        expect(subject['kubernetes']['container_name']).to eq('myappname')
        expect(subject['kubernetes']['replication_controller']).to eq('kube-testnometadata')
        expect(subject['kubernetes']['log_format_stderr']).to eq('default')
      end
    end
  end
